...
import { Checkbox, FileUploadSingle, OptionType, SMSelectDropDown, TextField, Typography } from "@/components";
...
export const AcademicBackground: React.FC<AcademicBackgroundProps> = ({
  ...
  const filterYears = (selectedFromYear: any) => {
    const yearsAboveSelectedFromYear = options.filter(year => 
      year.label >= selectedFromYear.label
    );
          ...
          <Checkbox
            ...
            onSelect={() => {
              if (!checked) {
                ...
                setValue(`academicBackground.${index}.to` as keyof ModifiedAcademicBackgroundFormData, undefined);
                clearErrors(`academicBackground.${index}.to`);
                setPrevToValue(previousValue);
              } else {
                setValue(`academicBackground.${index}.to` as keyof ModifiedAcademicBackgroundFormData, prevToValue);
              }
