...
import { useGetAllJobTitlesQuery } from "@/redux/api";
import { IApiError } from "@/redux/api/genericInterface";
import { useDebounce } from "@/utils/useDebouncedInput";

import { AddOrEditJobTitle, ImportJobTitles, Table } from "./components";
...
const Page = () => {
  ...
  return (
    <>
      ...
      >
        <AddOrEditJobTitle closeModal={() => setOpen(false)} />
      </Modal>
      <Modal
        isOpen={openImport}
        ...
        mobileLayoutType="full"
      >
        <ImportJobTitles closeModal={() => setOpenImport(false)} />
      </Modal>
      ...
      {data?.data?.items && data?.data?.items?.length === 0 && !searchTerm && (
        <div className="flex h-full items-center justify-center">
          <EmptyPageState
            title="No Job Titles Defined Yet"
