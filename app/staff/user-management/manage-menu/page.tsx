...
import { useGetAllMenusQuery } from "@/redux/api";
import { IApiError } from "@/redux/api/genericInterface";
import { useDebounce } from "@/utils/useDebouncedInput";

import { AddOrEditMenu, ManageMenuTable } from "./components";
...
const Page = () => {
  ...
  return (
    <>
      ...
      >
        <AddOrEditMenu closeModal={() => setOpen(false)} />
      </Modal>
      ...
      {data?.data?.items && data?.data?.items?.length === 0 && !searchTerm && (
        <div className="flex h-full items-center justify-center">
          <EmptyPageState
            title="No Menu Yet"
            text="No menu added currently. Create menu to Organize claims."
            buttonGroup={
              <>
                <Button
                  variant={"primary"}
