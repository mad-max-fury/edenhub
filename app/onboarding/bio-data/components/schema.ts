...
const academicBackgroundSchema = yup.object().shape({
  ...
  from: yup.object().when("$hasChanges", {
    is: true,
    then: (schema) => schema.required("From year is required"),
    otherwise: (schema) =>
      schema.notRequired().shape({
        label: yup.string().required("Please select a year"),
        value: yup.number().required("Please select a year"),
      }),
  }),
      ...
      then: (schema) =>
        schema
          ...
          .test(
            ...
            (value) => {
              const file = value as File;
              return file ? checkIfPdfOrImageAreCorrectType(file) : false;
            },
          ),
      otherwise: (schema) => schema.notRequired(),
    }),
...
const professionalQualificationSchema = yup.object().shape({
  ...
  from: yup.object().when("$hasChanges", {
    is: true,
    then: (schema) =>
      schema.required("from date is required").shape({
        label: yup.string().required("Please select a year"),
        value: yup.number().required("Please select a year"),
      }),
    otherwise: (schema) => schema.notRequired(),
  ...
  to: yup.object().when("$hasChanges", {
    is: true,
    then: (schema) =>
      schema.required("to date is required").shape({
        label: yup.string().required("Please select a year"),
        value: yup.number().required("Please select a year"),
      }),
    otherwise: (schema) => schema.notRequired(),
      ...
      then: (schema) =>
        schema
          ...
          .test(
            ...
            "Wrong file type, ensure this is an image file!",
            (value: File | null) => {
              return value ? checkIfPdfOrImageAreCorrectType(value) : true;
            },
          ),
      otherwise: (schema) => schema.notRequired(),
    }),
...
export const bankDetailsSchema = yup.object().shape({
  ...
  bankId: yup
    .object()
    .shape({
      label: yup.string().required("please select a bank"),
      value: yup.string().required("please select a bank"),
    })
    .required("please select a bank"),
});

export const taxPensionsSchema = yup.object().shape({
  taxId: yup
    .string()
    .trim()
    .when("$hasChanges", {
  ...
  fundsAdministratorId: yup
    .object()
    .shape({
      label: yup.string().when("$hasChanges", {
        is: true,
        then: (schema) => schema.optional(),
        otherwise: (schema) => schema.optional(),
      }),
      value: yup.string().when("$hasChanges", {
        is: true,
        then: (schema) => schema.optional(),
        otherwise: (schema) => schema.optional(),
      }),
