...
import { FileUploadSingle, FormStepJumbotron, notify, RadioButton, SMSelectDropDown, TextField, Typography, ValidationText } from "@/components";
import { RootState } from "@/redux";
import { IPersonalInfoProps, useUpdateEmployeePersonalDataMutation } from "@/redux/api";
import { ILGAProps, IStateProps, useLazyGetAllLGAsQuery, useLazyGetAllStatesQuery } from "@/redux/api/countryStatesLGA";
import { setPersonalInformation } from "@/redux/api/employee/bioDataForm.slice";
import { IApiError } from "@/redux/api/genericInterface";
import { AgeEnum, ISelectItemProps, ISelectItemPropsWithValueGeneric } from "@/redux/api/interface";
import base64ToFile from "@/utils/decodeImageFromBase64";
import formatImageToBase64 from "@/utils/formatImageToBase64";
import { getErrorMessage } from "@/utils/getErrorMessges";
import { fieldSetterAndClearer, formatInputDate, formatSelectItems } from "@/utils/helpers";
...
export const PersonalInformationForm = ({
  ...
  const { personalInformation, employeeId, profilePicture, staffId } =
  ...
  const onSubmit = async (data: IPersonalInformationFormData) => {
    ...
    formData.append("oldUser", String(true));
        ...
        {isLGAList && (
            <div>
              <SMSelectDropDown
                ...
                searchable={true}
                disabled={!(alllGAs.length > 0) || isLoadingLGAs || isFetchingLGAs}
          ...
          <div className="md:grid md:grid-cols-12">
            <div className="col-span-3">
              ...
              <Typography
                ...
                className="mb-2"
              >
                <label>{"(File should be less than 1MB)"}</label>
              </Typography>
            </div>
